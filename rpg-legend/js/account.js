...
RPG.Account=(function(){
  ...
  async function upload(silent){if(!user||!RPG.Save.hasSave())return false;if(RPG.Save.isImportedBackup()){if(!silent)message('Este progresso veio de um arquivo editável e está no modo offline. Carregue o save oficial da nuvem para voltar ao modo online.',true);return false;}try{var data=RPG.Save.load();await request('/api/save',{method:'PUT',body:JSON.stringify({save:data,source:'game'})});if(!silent)message('Progresso salvo na nuvem.');return true;}catch(e){if(!silent)message(e.message,true);return false;}}
  async function download(){try{var data=await request('/api/save');if(!data.save){message('Esta conta ainda não possui progresso na nuvem.',true);return;}if(!window.confirm('Carregar o progresso da nuvem substituirá o progresso deste navegador. Continuar?'))return;localStorage.setItem(RPG.Save.KEY,JSON.stringify(data.save));RPG.Save.markOfficial();message('Progresso recuperado. Recarregando…');setTimeout(function(){window.location.reload();},500);}catch(e){message(e.message,true);}}
  async function restorePrevious(){try{var data=await request('/api/save/history');if(!data.versions||!data.versions.length){message('Ainda não existe uma versão anterior na nuvem.',true);return;}var version=data.versions[0],when=new Date(version.createdAt).toLocaleString('pt-BR');if(!window.confirm('Restaurar a versão da nuvem salva em '+when+'? O save atual também ficará guardado no histórico.'))return;var restored=await request('/api/save/restore',{method:'POST',body:JSON.stringify({id:version.id})});localStorage.setItem(RPG.Save.KEY,JSON.stringify(restored.save));RPG.Save.markOfficial();message('Versão anterior restaurada. Recarregando…');setTimeout(function(){window.location.reload();},500);}catch(e){message(e.message,true);}}
  ...
  function init(){
    ...
    el('accountLoginBtn').onclick=function(){enter('login');};el('accountRegisterBtn').onclick=function(){enter('register');};el('cloudSaveBtn').onclick=function(){upload(false);};el('cloudLoadBtn').onclick=download;el('cloudRestoreBtn').onclick=restorePrevious;el('accountLogoutBtn').onclick=logout;refresh();
  }
