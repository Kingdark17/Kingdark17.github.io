...
RPG.Combat = (function(){
  ...
  function startEncounterChoice(cell){
    ...
    RPG.UI.setSceneMessage('Um '+cell.monster.name+' aparece! O que voce faz?');
  ...
  function exitCombat(cell, victory, gotLoot){
    ...
    document.getElementById('combatScene').classList.add('hidden');
    document.getElementById('npcCard').style.display = 'none';
    if(victory){ RPG.UI.setSceneMessage('Voce venceu a batalha!' + (gotLoot? ' Encontrou um item entre os destrocos.':'')); }
