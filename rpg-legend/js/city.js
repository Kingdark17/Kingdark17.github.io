/* =========================================================
   ...
   Geracao do mapa da cidade inicial (loja, ferreiro, taverna,
   quadro de missoes, NPCs e portao da masmorra).
   ========================================================= */
var RPG = window.RPG || {};
...
RPG.City = (function(){
  ...
  function generate(state){
    ...
    var res = RPG.MapUtil.carveWalk(rows, cols, 0.5);
    var grid = res.grid, floorCells = res.floorCells;
    grid.forEach(function(row){ row.forEach(function(cell){
      if(cell.type==='wall'){ cell.type='building'; }
      if(cell.type==='floor'){ cell.type='street'; }
    }); });
    ...
    grid.forEach(function(row){ row.forEach(function(cell){
      if(cell.type === 'npc'){ cell.npc = RPG.MapUtil ? pickNpc() : null; }
    }); });
  ...
  // Trata a entrada do jogador em uma celula da cidade. Retorna true se tratou.
  ...
  return { generate: generate, iconFor: iconFor, entryText: entryText, handleEnter: handleEnter };
})();
