...
RPG.Multiplayer = (function(){
  ...
  function status(){
    ...
    if(session.connected)el.textContent='Sala '+session.room+' · '+(session.turn===session.role?'SUA VEZ':'vez do parceiro');
  }
  ...
  function captureProfile(){
    ...
    session.profiles[session.role]={name:session.name,hero:RPG.state.hero,inventory:RPG.state.inventory||[]};
  }
  function snapshot(){
    if(!RPG.state||!RPG.state.hero)return null;captureProfile();var s=RPG.state;
    return {profiles:session.profiles,party:s.party,quests:s.quests,floor:s.floor,mapMode:s.mapMode,map:s.map,mapRows:s.mapRows,mapCols:s.mapCols,pos:s.pos,mode:s.mode,pendingTarget:s.pendingTarget,pendingMonsterPos:s.pendingMonsterCell?{x:s.pendingMonsterCell.x,y:s.pendingMonsterCell.y}:null,soundOn:s.soundOn};
  }
  ...
  function applyState(data){
    ...
    var s=RPG.state;
    ['party','quests','floor','mapMode','map','mapRows','mapCols','pos','mode','pendingTarget','soundOn'].forEach(function(k){if(data[k]!==undefined)s[k]=data[k];});
    s.hero=own.hero;s.inventory=own.inventory||[];s.pendingMonsterCell=null;
  ...
  function finishGuestCreation(state){
    session.guestCreating=false;session.profiles[2]={name:session.name,hero:state.hero,inventory:state.inventory||[]};
  ...
  function commit(){
    ...
    session.turn=session.role===1?2:1;send({type:'state',state:snapshot(),turn:session.turn,role:session.role});status();
  }
  function canAct(){return !session.connected||session.turn===session.role;}
  function guard(e){
    if(!session.connected||canAct())return;var target=e.target;
    if(target&&(target.closest('#multiplayerModal')||target.closest('#multiplayerStatus')))return;
    if(session.guestCreating)return;
    var blocked=target&&(target.id==='btnNewGame'||target.id==='btnContinueGame'||target.id==='startAdventureBtn');
    if(blocked||(RPG.state&&(RPG.state.screen==='game'||RPG.state.screen==='creation'))){e.preventDefault();e.stopImmediatePropagation();message('Aguarde a vez do outro jogador.',true);}
  }
  function init(){
  ...
  return {commit:commit,canAct:canAct,isGuestCreating:function(){return session.guestCreating;},finishGuestCreation:finishGuestCreation,session:session};
})();
