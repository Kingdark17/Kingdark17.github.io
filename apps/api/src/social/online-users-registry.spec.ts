...
describe('OnlineUsersRegistry', () => {
  it('marca online ao registrar e offline quando a última conexão sai', () => {
    const registry = new OnlineUsersRegistry();
    const aba1 = fakeConnection('c1');
    const aba2 = fakeConnection('c2');

    expect(registry.isOnline(7)).toBe(false);

    registry.add(7, aba1);
    registry.add(7, aba2);
    expect(registry.isOnline(7)).toBe(true);
  ...
  it('reautenticar a mesma conexão com outro usuário move a presença', () => {
    const registry = new OnlineUsersRegistry();
    ...
    expect(registry.isOnline(7)).toBe(false);
    expect(registry.isOnline(9)).toBe(true);
