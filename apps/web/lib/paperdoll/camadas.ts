...
export interface Vestimenta {
  /** Id da raça (`RACES[].id`). */
  raca: string | null;
  /** Id do template da arma (`CLASSES[].weaponTemplate`). */
  arma?: string | null;
}
 ...
 *
 * A ordem é a de vestir: corpo, calça, roupa, cabelo, arma na frente de
 * tudo. É a mesma que o `monta-paperdoll.mjs` recebe na linha de comando,
 * e trocá-la aqui sem trocar lá faz a conferência mentir.
 */
export function montarCamadas({ raca, arma }: Vestimenta): string[] {
